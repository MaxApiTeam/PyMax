...
``Voice``
   ...
   конвертирует другие аудиоформаты. Длительность можно передать вручную или
   определить автоматически через extra ``video``.
...
``duration`` для ``Voice`` и ``VideoNote`` задается в миллисекундах. Если
длительность не передана, установите дополнительную зависимость для её
...
4. Для ``File`` и обычного ``Video`` ждет служебное событие готовности до
   60 секунд.
...
``Voice`` и ``VideoNote`` отправляются сразу после HTTP-upload. Если Max
ответил ``attachment.not.ready``, PyMax ждет событие готовности до 60 секунд и
повторяет отправку сообщения один раз.

Общий timeout HTTP-загрузки ``File``, ``Video``, ``Voice`` и ``VideoNote``
задается через ``ExtraConfig(upload_timeout=...)`` в секундах; значение по
умолчанию - ``900``. Он не применяется к ``Photo`` и не меняет отдельный
60-секундный лимит ожидания обработки. ``0`` и отрицательные значения
отключают общий timeout, но не отдельный 60-секундный socket/read timeout.
...
``Automatic video duration detection requires the 'video' extra``
   Для ``Voice`` или ``VideoNote`` передайте ``duration`` в миллисекундах либо
   установите ``maxapi-python[video]``.
